...
final class WorkflowPlannerTests: XCTestCase {
    func testImportPlanPointsAtConfiguredPathsAndLockedRcloneCommands() throws {
        try withTemporaryDirectory { root in
            ...
            let copyCommand = try XCTUnwrap(plan.steps.first { $0.title == "Immutable Copy" }?.command)
            XCTAssertEqual(Array(copyCommand.prefix(4)), [
                "/opt/homebrew/bin/rclone",
                "copy",
                root.appendingPathComponent("Card").path,
                root.appendingPathComponent("Archive").path
            ])
            ...
            let checkCommand = try XCTUnwrap(plan.steps.first { $0.title == "Verify Archive" }?.command)
    ...
    func testImmichPlanNormalizesEndpointAndRequiresKeyGate() throws {
        try withTemporaryDirectory { root in
            ...
            XCTAssertEqual(readyPlan.steps.first { $0.title == "Upload Asset" }?.endpoint, "http://photos.local:2283/api/assets")
            XCTAssertEqual(readyPlan.steps.first { $0.title == "Upload Asset" }?.detail, "multipart fields: assetData, fileCreatedAt, fileModifiedAt")
        }
    }
