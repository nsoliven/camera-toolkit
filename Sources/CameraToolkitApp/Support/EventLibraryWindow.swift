...
private struct EventLibraryView: View {
        ...
        if let event = selectedEvent {
            VStack(spacing: 10) {
                ...
                HStack(spacing: 10) {
                    ...
                    eventFolderButton("NAS Edited", systemImage: "externaldrive", url: eventFolderURLs(event).nasEdited)
                    Spacer()
                }
            }
            .padding(14)
    ...
    private var assetTable: some View {
        Table(rows, selection: $selectedAssetID) {
            ...
            TableColumn("NAS Originals") { row in
        ...
        if let row = selectedRow {
            VStack(alignment: .leading, spacing: 8) {
                ...
                HStack(spacing: 18) {
                    ...
                    pathLink("NAS", url: row.archiveURL, presence: row.archivePresence)
                }
            }
    ...
    private func eventFolderURLs(_ event: SavedCameraEvent) -> (photomator: URL, exports: URL, nasEdited: URL) {
    ...
    private func refreshEvent() async {
        ...
        let suffix = offlineCount > 0
            ...
            : ""
        statusText = "Found \(cardCount) on source folders, \(bufferCount) in the buffer, and \(archiveCount) in NAS Originals.\(suffix)"
    }
