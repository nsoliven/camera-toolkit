...
public enum EventNamePolicy {
    ...
    public static func validate(_ rawName: String) -> EventNameValidation {
        ...
        if !invalidCharacters.isEmpty {
            ...
            return invalid(
                normalizedName: normalizedName,
                message: "The \(renderedCharacters) \(noun) can’t be used in NAS folder names. Use a dash instead.",
                suggestion: suggestion
            )
        }
        ...
        if normalizedName.last == "." {
            ...
            return invalid(
                normalizedName: normalizedName,
                message: "An event name can’t end with a period on the NAS. Remove it or use a dash.",
